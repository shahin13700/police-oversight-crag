...
# ── Pipeline ─────────────────────────────────────────────────────────
@st.cache_resource(show_spinner="Loading CSPA, O.Regs and LECA legislation and building index...")
def initialise_pipeline():
    from src.ingestion.pipeline import run_ingestion
    from src.agent.graph import build_graph
    chunks = run_ingestion()
    return build_graph(chunks), chunks
...
try:
    graph,chunks=initialise_pipeline()
    pipeline_ready=True
except Exception as e:
    st.error(f"Failed to initialise pipeline: {e}")
    st.info("Make sure CSPA_2019.docx is in data/raw/ and GROQ_API_KEY is set in .env")
...
if st.session_state.pending_question:
    ...
    with st.chat_message("assistant", avatar=_book_path):
        with st.spinner("Searching legislation..."):
            ...
            r=run_query(graph,q); a=r.get("answer","No answer."); ret=r.get("retrieved_chunks",[])
    ...
    if prompt:=st.chat_input("Ask a question about Ontario police oversight..."):
        ...
        with st.chat_message("assistant", avatar=_book_path):
            with st.spinner("Searching legislation..."):
                try:
                    from src.agent.graph import run_query
                    r=run_query(graph,prompt); a=r.get("answer","No answer."); ret=r.get("retrieved_chunks",[])
                    conf=r.get("confidence","Low")
